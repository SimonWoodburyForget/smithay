...
use drm::control::{crtc, connector, encoder, framebuffer, plane, Device as ControlDevice, Mode, ResourceHandles};
...
use crate::backend::egl::{Error as EglError, EGLError as RawEGLError, wrap_egl_call};
...
pub enum Error<U: std::error::Error + std::fmt::Debug + std::fmt::Display + 'static> {
    ...
    /// Stream flip failed
    #[error("Stream flip failed ({0:#x})")]
    StreamFlipFailed(i32)
}
        ...
        L: Into<Option<::slog::Logger>>,
    {
        ...
        let device = unsafe {
            ...
            if let Err(_err) = has_extensions(&dp_extensions, &["EGL_EXT_device_base"]) {
                has_extensions(&dp_extensions, &["EGL_EXT_device_enumeration", "EGL_EXT_device_query"]).map_err(|_| Error::DeviceIsNoEGLStreamDevice)?;
            }
            ...
            devices.into_iter().find(|device| *device != ffi::egl::NO_DEVICE_EXT && {
                ...
                let path = {
                    let p = ffi::egl::QueryDeviceStringEXT(*device, ffi::egl::DRM_DEVICE_FILE_EXT as i32);
                    if p.is_null() {
                        String::new()
                    } else {
                        let p = CStr::from_ptr(p);
...
impl<D: RawDevice + ControlDevice + 'static> Device for EglStreamDevice<D> {
    ...
    fn create_surface(&mut self, crtc: crtc::Handle, mode: Mode, connectors: &[connector::Handle]) -> Result<EglStreamSurface<D>, Error<<<D as Device>::Surface as Surface>::Error>> {
        info!(self.logger, "Initializing EglStreamSurface");

        let drm_surface = Device::create_surface(&mut self.raw, crtc, mode, connectors)
            .map_err(Error::Underlying)?;

        // initialize a buffer for the cursor image
        let cursor = Cell::new(Some((
            self.raw.create_dumb_buffer(
                (1, 1),
